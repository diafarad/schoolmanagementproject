...
require_once "../public/web/rooting.php";
    ...
    {
        $ok = addEvaluation($_POST['libelleEval'],$_POST['typeEval'],$_POST['semEnCours'],$_POST['anAc'],$_POST['idCl'],$_POST['matiereEval']);
        header("location:..?page=evaluation&resultA=$ok");
    }
    ...
    if(isset($_GET['idcl_note']) && isset($_GET['anneeAc_note']) && isset($_GET['idEval']) && isset($_GET['sem']))
    {
        ...
        while($res=mysqli_fetch_row($result))
        {
            ...
            $output .= '<td style="text-align:center;">'.$res[7].'<input type="hidden" name="hidden_genre[]" id="genre'.$count.'" value="'.$res[7].'"/></td>';
    ...
    if(isset($_GET['ideval_note']) && isset($_GET['anneeAcad_note']) && isset($_GET['sem_note']) && isset($_GET['idcl_note']))
    {
        ...
        while($res=mysqli_fetch_row($result))
        {
            ...
            $output .= '<td style="text-align:center;">'.$res[17].'</td>';
            $output .= '</tr>';
        }
        echo $output;
    }
